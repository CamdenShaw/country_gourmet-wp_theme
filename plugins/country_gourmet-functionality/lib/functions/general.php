...
class RF_General {
     ...
     * @since 1.0.0
     * 
     * This prevents you from being prompted to update if there's a public plugin with the same name.
     * 
     * @author Mark Jaquith
    ...
    /**
     * Remove unnecessary sub-menu links
     * 
     * @since 1.0.0
     */
    ...
    public function set_updated_messages( $messages ) {
        ...
        $messages[$post_tpe] = array(
            ...
            5 => isset( $GET['revision'] ) ? sprintf( __( $singular . 'restored to revision from %s' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
            6 => sprintf( __( $singular . ' published. <a href="%s">View ' . strtolower( $singular ) . '</a>' ), esc_url( get_permalink( $post_ID ) ) ),
            7 => __( $singular . ' saved.' ),
            8 => sprintf( __( $singular . ' submitted. <a href"%s">View ' . strtolower( $singular ) . '</a>' ), esc_url(get_permalink( $post_ID ) ) ),
